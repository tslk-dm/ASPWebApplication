...
namespace Lesson3_homework.Data.Implamentation
{
    public class PersonRepo : IPersonRepo
    {
        ...
        {
            if (persons.RemoveAll(i => i.Id == id) == 1)
            {
                return true;
            }
            else 
        ...
        {
            return persons.FirstOrDefault(i => i.FirstName == name);
        }

        public int GetIdForUpdate(string firstName, string lastName)
        {
            var person = persons.First(i => (i.FirstName == firstName) & (i.LastName == lastName));
        ...
        public int GetNewId()
        {
            var newId = persons[persons.Count - 1].Id + 1;
            return newId;
        }

        public IEnumerable<Person> GetPagination(int skip, int take)
        {
            var result = persons.Skip(skip).Take(take);
            return result;
        }
